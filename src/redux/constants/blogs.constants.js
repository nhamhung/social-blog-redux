export const GET_BLOGS_DATA_REQUEST = "GET_BLOGS_DATA_REQUEST";
export const GET_BLOGS_DATA_SUCCESS = "GET_BLOGS_DATA_SUCCESS";
export const GET_BLOGS_DATA_FAILURE = "GET_BLOGS_DATA_FAILURE";

export const WRITE_BLOG_REQUEST = "WRITE_BLOG_REQUEST";
export const WRITE_BLOG_SUCCESS = "WRITE_BLOG_SUCCESS";
export const WRITE_BLOG_FAILURE = "WRITE_BLOG_FAILURE";

export const EDIT_BLOG_REQUEST = "EDIT_BLOG_REQUEST";
export const EDIT_BLOG_SUCCESS = "EDIT_BLOG_SUCCESS";
export const EDIT_BLOG_FAILURE = "EDIT_BLOG_FAILURE";

export const SELECT_SINGLE_BLOG = "SELECT_SINGLE_BLOG";


export const GET_A_BLOG_REQUEST = 'GET_A_BLOG_REQUEST';
export const GET_A_BLOG_SUCCESS = 'GET_A_BLOG_SUCCESS';
export const GET_A_BLOG_FAILURE = 'GET_A_BLOG_FAILURE';
